...
class _AdminNotificationsPageState extends State<AdminNotificationsPage> {
  ...
  Future<void> _selectNotification(Map<String, dynamic> notification) async {
    ...
    setState(() {
      ...
      _selectedStatus = _normalizeStatus(
        (notification['complaintStatus'] ?? 'Opened').toString(),
      );
    });
    ...
    setState(() {
      ...
      if (!complaint.containsKey('error')) {
        _selectedStatus = _normalizeStatus(
          (complaint['status'] ?? _selectedStatus).toString(),
        );
      }
    });
  ...
  Future<void> _updateComplaintStatus() async {
    if (_selectedComplaint == null || _selectedComplaint!.containsKey('error')) {
      return;
    }

    final complaintId = (_selectedComplaint!['_id'] ?? _selectedComplaint!['id'])
        .toString();
    ...
    final response = await ApiService.updateComplaint(complaintId, {
    ...
    if (response.containsKey('error')) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(response['error']?.toString() ?? 'Failed to update status'),
    ...
    if (_selectedNotification != null) {
      await _selectNotification(_selectedNotification!);
    }
  }
  ...
  Widget _buildComplaintDetail() {
    ...
    final customerName = (complaint['customerName'] ?? '').toString();
    final customerEmail = (complaint['customerEmail'] ?? complaint['submittedBy'] ?? '')
        .toString();
    final customerPhone = (complaint['customerPhone'] ?? '').toString();
    final customerAddress = (complaint['customerAddress'] ?? '').toString();
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          Text('Priority: ${(complaint['priority'] ?? '-').toString()}'),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: _isUpdatingStatus ? null : _updateComplaintStatus,
